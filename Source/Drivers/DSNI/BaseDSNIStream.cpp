...
BaseDSNIStream::BaseDSNIStream(DSNIStreamImpl* pStreamImpl)
...
BaseDSNIStream::~BaseDSNIStream()
{
  stop();
  m_pStreamImpl->removeStream(this);
}

OniStatus BaseDSNIStream::start()
{
...
void BaseDSNIStream::stop()
{
  m_running = FALSE;
  m_pStreamImpl->stop();
}

OniStatus BaseDSNIStream::getProperty(int propertyId, void* data, int* pDataSize)
{
...
OniStatus BaseDSNIStream::setProperty(int propertyId, const void* data, int dataSize)
{
  OniStatus status = ONI_STATUS_NOT_SUPPORTED;
...
OniBool BaseDSNIStream::isPropertySupported(int propertyId)
{
...
OniStatus BaseDSNIStream::SetVideoMode(OniVideoMode* videoMode)
{
...
OniStatus BaseDSNIStream::GetVideoMode(OniVideoMode* pVideoMode)
{
  *pVideoMode = m_videoMode;
  return ONI_STATUS_OK;
}

OniStatus BaseDSNIStream::SetCropping(OniCropping* cropping)
{
  m_cropping = *cropping;
  return ONI_STATUS_OK;
}

OniStatus BaseDSNIStream::GetCropping(OniCropping* cropping)
{
  *cropping = m_cropping;
  return ONI_STATUS_OK;
}
