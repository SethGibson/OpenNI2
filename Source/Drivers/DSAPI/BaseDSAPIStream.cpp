...
BaseDSAPIStream::BaseDSAPIStream(DSAPIStreamImpl* pStreamImpl)
...
BaseDSAPIStream::~BaseDSAPIStream()
{
  stop();
  m_pStreamImpl->removeStream(this);
}

OniStatus BaseDSAPIStream::start()
{
...
void BaseDSAPIStream::stop()
{
  m_running = FALSE;
  m_pStreamImpl->stop();
}

OniStatus BaseDSAPIStream::getProperty(int propertyId, void* data, int* pDataSize)
{
...
OniStatus BaseDSAPIStream::setProperty(int propertyId, const void* data, int dataSize)
{
  OniStatus status = ONI_STATUS_NOT_SUPPORTED;
...
OniBool BaseDSAPIStream::isPropertySupported(int propertyId)
{
...
OniStatus BaseDSAPIStream::SetVideoMode(OniVideoMode* videoMode)
{
...
OniStatus BaseDSAPIStream::GetVideoMode(OniVideoMode* pVideoMode)
{
  *pVideoMode = m_videoMode;
  return ONI_STATUS_OK;
}

OniStatus BaseDSAPIStream::SetCropping(OniCropping* cropping)
{
  m_cropping = *cropping;
  return ONI_STATUS_OK;
}

OniStatus BaseDSAPIStream::GetCropping(OniCropping* cropping)
{
  *cropping = m_cropping;
  return ONI_STATUS_OK;
}
