...
DepthDSAPIStream::DepthDSAPIStream(DSAPIStreamImpl* pStreamImpl)
  : BaseDSAPIStream(pStreamImpl)
{
...
DepthDSAPIStream::~DepthDSAPIStream()
{
  delete[] m_colorSpaceCoords;
  delete[] m_registeredDepthMap;
}

void DepthDSAPIStream::frameReady(void* data, int width, int height, double timestamp)
{
...
OniStatus DepthDSAPIStream::getProperty(int propertyId, void* data, int* pDataSize)
{
  ...
  default:
    status = BaseDSAPIStream::getProperty(propertyId, data, pDataSize);
    break;
  }
...
OniBool DepthDSAPIStream::isPropertySupported(int propertyId)
{
  ...
  case ONI_STREAM_PROPERTY_MIRRORING:
    status = TRUE;
  default:
    status = BaseDSAPIStream::isPropertySupported(propertyId);
    break;
  }
  return status;
}

void DepthDSAPIStream::notifyAllProperties()
{
  XnInt nInt;
  ...
  BaseDSAPIStream::notifyAllProperties();
}

void DepthDSAPIStream::copyDepthPixelsStraight(const UINT16* data_in, int width, int height, OniFrame* pFrame)
{
...
void DepthDSAPIStream::copyDepthPixelsWithImageRegistration(const UINT16* data_in, int width, int height, OniFrame* pFrame)
{
  // Copy the depth pixels to OniDriverFrame
